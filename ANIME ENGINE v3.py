...
pygame.display.set_caption("Anime Film Engine v5")
...
class AuraParticle(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.life <= 0:
            ...
            self.image.set_alpha(int(self.life / 50 * 150))
...
# Clouds
clouds = [{"x": random.randint(0, WIDTH), "y": random.randint(50,200), "size": random.randint(80,150), "speed": random.uniform(5,20)/FPS} for _ in range(8)]

# Stars
stars = [{"x": random.randint(0, WIDTH), "y": random.randint(0, HEIGHT//2), "alpha": random.randint(100,255)} for _ in range(120)]
...
while running:
    ...
    # Dynamic day-night (0-60s mapped to 24h fraction)
    day_fraction = t / TOTAL
    # Sky color transition
    if day_fraction < 0.25:  # Sunrise
        sky = (
            int(120 + (255-120)*(day_fraction/0.25)),
            int(190 + (200-190)*(day_fraction/0.25)),
            int(255 - 155*(day_fraction/0.25))
        )
    elif day_fraction < 0.5:  # Day
        sky = (255, 200, 100 + int(155*(0.5-day_fraction)/0.25))
    elif day_fraction < 0.75:  # Sunset
        sky = (255 - int(245*(day_fraction-0.5)/0.25), 170 - int(150*(day_fraction-0.5)/0.25), 120 - int(120*(day_fraction-0.5)/0.25))
    else:  # Night
        sky = (10,10,20)
    ...
    # Scene determination for effects
    if t < 15:
        scene = 1
    elif t < 30:
        scene = 2
    ...
    if day_fraction >= 0.75 or day_fraction < 0.25:
        for s in stars:
            surface.set_at((s["x"], s["y"]), (255,255,255))
